  ...
  function _call(
    ...
    xhr.onload = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        if (
          Object.prototype.toString.call(successcallback) ===
          "[object Function]"
        )
          successcallback(JSON.parse(xhr.responseText));
      } else {
        if (
          Object.prototype.toString.call(errorcallback) === "[object Function]"
        )
          errorcallback();
      }
    };
    xhr.onloadend = function () {
      if (
        Object.prototype.toString.call(alwayscallback) === "[object Function]"
      )
        alwayscallback();
    };
    xhr.send(data);
  }
  function checkRecommendeds(
    product_id,
    successcallback,
    errorcallback,
    alwayscallback
    ...
    if (Object.prototype.toString.call(product_id) === "[object Array]")
      _call(
        "GET",
        "/js/check_list_recommendeds?product_ids=" + product_id,
        {},
        successcallback,
        errorcallback,
        alwayscallback
    ...
    comboCall: function (
      ...
      xhr.onload = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
          ...
          if (
            Object.prototype.toString.call(errorcallback) ===
            "[object Function]"
          )
            errorcallback();
        }
    ...
    comboRender: function (result, id, title) {
      ...
      $.each(result, function (key, value) {
            ...
            success: function (response) {
              ...
              imm =
                response.featured_image != null
                  ? Haravan.resizeImage(response.featured_image, "small")
                  : "https://hstatic.net/0/0/global/noDefaultImage6_large.gif";
              variant = is_apply_variant
                ? v.apply_productvariants[0].id
                : response.variants[0].id; //Fix
              pid = response.id;
            },
          });
          ...
          if (is_none_promotion == false) {
            ...
            if (fixedPrice == xprice) {
              typeHtml +=
                "<p><b>" +
                v.product_name.split("|")[0] +
                "</b></p><p class='discount-promotion-price'><b>" +
                fixedPrice
                  .toString()
              ...
              typeHtml +=
                ...
                "₫" +
                "</b><del>" +
                xprice.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") +
                "₫" +
                "</del></p>";
            }
          } else {
            ...
            typeHtml +=
              ...
              v.product_name.split("|")[0] +
              "</b></p><p class='discount-promotion-price'><b>" +
              fixedPrice.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") +
              "₫" +
              "</b></p>";
          }
          var html =
            '<tr haravan-promotion><td><label class="combo-product" data-pid="' +
            pid +
            '" data-countbuy="' +
            v.quantity +
            '" data-id="' +
            variant +
            '"><img class="combo-img" src="' +
            imm +
            '"><p>' +
            typeHtml +
            "</p></label></td></tr>";
        ...
        giftHtml +=
          "<div class='combo-w " +
          (key == 0 ? "checked" : "") +
          "'><div class='combo-list'><table>" +
          itemCombo +
          "</table></div></div>";
        titleHtml =
          "<p class='name-combo'><span>" +
          value.name_combo +
          "</span></p><p class='combo-title'>Khi mua " +
          noti.substr(0, noti.length - 1) +
          "sẽ được áp dụng giảm giá như sau</p>";
      });
      ...
      $("#combo-program .combo-w.checked .combo-product").each(function (k, e) {
        if (parseInt($(this).attr("data-pid")) == pid) {
          data[k] = {
            vid: vid,
            quan:
              parseInt($(this).attr("data-countbuy")) *
